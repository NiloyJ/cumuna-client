...
const BlogCard = ({ job, blogData, setJobs }) => {
    ...
    return (
        <div className="card w-full bg-white text-black shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200">
            <figure className="px-6 pt-6 pb-0">
                <img
                    src={thumbnailUrl || 'https://via.placeholder.com/600x400?text=Blog+Thumbnail'}
                    alt={title}
            ...
            <div className="card-body p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">{author || 'Anonymous'}</span>
                    </div>
                </div>

                <h2 className="card-title text-lg sm:text-xl font-bold mb-3 line-clamp-2 text-black">{title}</h2>
