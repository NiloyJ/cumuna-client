...
const FlagshipEvent = () => {
    ...
    if (loading) {
        return (
            <Box display="flex" justifyContent="center" alignItems="center" minHeight="300px" bgcolor="white">
        ...
        <div className="bg-white min-h-screen w-full">
            <section className="conference-section max-w-7xl mx-auto px-6 py-20 lg:py-28 relative overflow-hidden">
                <div className="conference-container flex flex-col items-center text-center relative z-10">
                    <h1 className="conference-title text-4xl md:text-5xl font-bold text-black mb-6">
                        CUMUNA Conference Events
                    </h1>

                    <p className="conference-subtitle text-lg md:text-xl text-black max-w-3xl mb-12">
                        ...
                        <div className="conference-highlights flex flex-wrap justify-center gap-6 mb-12">
                            {filteredEvents.map((event) => {
                                ...
                                return (
                                    <div 
                                        key={event._id} 
                                        className="highlight-card bg-white p-8 rounded-xl shadow-md border border-gray-200 w-full sm:w-80 hover:-translate-y-[5px] transition-transform"
                                    >
                                        <img 
                                            src={event.bannerUrl} 
                                            alt="Event Banner" 
                                        ...
                                        />
                                        <h3 className="text-xl font-semibold text-black mb-2">{event.theme}</h3>
                                        <div className="text-left space-y-2">
                                            <p className="text-gray-700">
                                                <span className="font-semibold text-black">Dates:</span> {event.dates || 'Not specified'}
                                            </p>
                                            <p className="text-gray-700">
                                                <span className="font-semibold text-black">Year:</span> {year}
                                            </p>
                                            <p className="text-gray-700">
                                                <span className="font-semibold text-black">Committees:</span> {event.totalCommittees || 0}
                                            </p>
                                            <p className="text-gray-700">
                                                <span className="font-semibold text-black">Delegates:</span> {event.totalDelegates || 0}
                                            </p>
                                            <p className="text-gray-700">
                                                <span className="font-semibold text-black">International:</span> {event.internationalDelegates || 0}
                                            </p>
                                        </div>
